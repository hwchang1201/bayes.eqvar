...
stepwise <- function(V, q, a, sum_sse_rest, c3, c4){
...
top_down <- function(V, sse_pvector, c3, c4){
  # V : the gram matrix of crossprod(X), p x p matrix.
  ...
  for (t in 2:p){
    ...
    for (i in 1:p){
      ...
      s = c(perm, i)
      out = stepwise(V, i, perm, sum(sse_pvector[-i]), c3, c4)
      sses[i] = out$sse
      G1[out$model, i] = 1
    }
...
top_down_iterative <- function(V, c3, c4){
  # V : the gram matrix of crossprod(X), p x p matrix.
  # c3, c4 : model parameters
  p = ncol(V)
  cat("BayTD iteration 1\n")
  td = top_down(V, diag(V), c3, c4)
  perm = numeric(p)
  count = 1
...
stepwise_range <- function(V, q, a, min_sse_pvector, max_sse_pvector, c3, c4){
  # V : the gram matrix of crossprod(X), p x p matrix.
  # q : index of target node.
...
candidate_parents <- function(V, TO, min_sse_pvector, max_sse_pvector, c3, c4){
  # V : the gram matrix of crossprod(X), p x p matrix.
  ...
  for (j in 1:p){
    ...
    a = NULL
    if (j >= 2){a = TO[1:(j-1)]}
    parents[[i]] = stepwise_range(V, i, a, min_sse_pvector, max_sse_pvector, c3, c4)
  }
...
order_mcmc <- function(V, B, N, c3, c4){
  ...
  init = top_down_iterative(V, c3, c4)
  perm = init$TO
  ITD = init$DAG # output 1. ITD
  min_sse_pvector = 1/diag(solve(V))
  max_sse_pvector = diag(V)
  sup_parents = candidate_parents(V, perm, min_sse_pvector, max_sse_pvector, c3, c4)
  ...
  for (t in 1:N){
    ...
    pa_b = new_perm[1:(b-1)]
    parents_a = stepwise_range(V, new_perm[a], pa_a, min_sse_pvector, max_sse_pvector, c3, c4)
    parents_b = stepwise_range(V, new_perm[b], pa_b, min_sse_pvector, max_sse_pvector, c3, c4)
...
order_mcmc_mixing <- function(V, N, method, TO, c3, c4){
  # V : the gram matrix of crossprod(X), p x p matrix.
  # N: the number of total MC iterations.
  ...
  if((method=='ran-to-ran')|(method=='ran_trs')){
    for(t in 1:N){
      ...
      new_parents = candidate_parents(V, new_perm, min_sse_pvector, max_sse_pvector, c3, c4)
      new_dag = order_to_DAG(new_parents, c3, c4)
